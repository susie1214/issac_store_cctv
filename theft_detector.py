...
PROMPT = """이것은 의류/가방/신발 매장의 CCTV 캡처 이미지입니다.

아래 도난 의심 행동이 보이면 'YES', 아니면 'NO'만 답하세요:

도난 의심 행동:
- 옷·가방·액세서리를 자신의 옷 안쪽이나 개인 가방에 숨기는 행동
- 행거나 진열대에서 물건을 집어 빠르게 몸 쪽으로 감추는 행동
- 주변을 두리번거리며 누가 보는지 확인하면서 물건을 만지는 행동
- 가격표나 태그를 몰래 제거하는 행동
- 탈의실 없이 옷을 껴입으려는 행동
